...
const renderNavBar = (nav, menuList) => {
  if (!nav) {
...
// const renderNavBar = (menuList) => {
//   // 객체가 아닌, 배열의 경우 아래 map 함수에서 가독성이 떨어짐
//   const menuHTML = menuList.map(
//     (menu) => `<li><a href=${menu.path}><strong>${menu.label}</strong></a></li>`
//   );

//   return `<ul>${menuHTML.join('')}</ul>`;
// };
