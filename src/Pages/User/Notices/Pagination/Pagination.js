import renderNoticesList from "../JS/RenderNoticesList.js";
...
const renderCurrentPageData = (data) => {
  ...
  renderNoticesList(currentPageData); // 데이터 렌더링 함수 호출
};
...
const renderPagination = (data) => {
  ...
  for (let i = startPage; i <= endPage; i++) {
    ...
    pageButton.className = paginationStyle.pageNumber__btn; // 버튼 스타일 클래스 설정
    if (i === currentPage) {
      pageButton.classList.add(paginationStyle.active); // 현재 페이지 버튼 활성화 클래스 추가
    }
